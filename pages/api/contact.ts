...
const CONTACT_MESSAGE_FIELDS: Contact = {
  name: 'Имя',
  email: 'Почта',
  tel: 'Телефон',
  message: 'Сообщение',
...
const generateEmailContent = (data: Contact) => {
  ...
  const htmlData = Object.entries(data).reduce((str, [key, val]) => (`${str}<h3 class="form-heading" align="left">
       ${CONTACT_MESSAGE_FIELDS[key]}</h3><p class="form-answer" align="left">${val}</p>`), '');
...
.form-heading{color: #2a2a2a; font-family: "Helvetica Neue", "Helvetica", "Arial", sans-serif; 
font-weight: 400; text-align: left; line-height: 20px; font-size: 18px; margin: 0 0 8px; padding: 0;}
.form-answer{color: #2a2a2a; font-family: "Helvetica Neue", "Helvetica", "Arial", sans-serif; font-weight: 300; 
text-align: left; line-height: 20px; font-size: 16px; margin: 0 0 24px; padding: 0;}
...
const handlerContact = async (req: NextApiRequest, res: NextApiResponse<Contact>) => {
  if (req.method === 'POST') {
    const data = req.body;
